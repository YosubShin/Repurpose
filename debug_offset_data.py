...
def check_json_annotations(json_file):
    ...
    for i, (video_id, video_data) in enumerate(list(data.items())[:3]):
        ...
        for j, segment in enumerate(video_data['timestamps'][:3]):
            ...
            if segment[0] > 1000:
                print(f"    WARNING: Large values suggest these might be in milliseconds or frames")
    ...
    for idx in range(min(num_samples, len(dataset))):
        ...
        if len(positive_idx) > 0:
            ...
            for i in range(min(5, len(positive_idx))):
                ...
                print(f"    Is integer? Left: {left_off == int(left_off)}, Right: {right_off == int(right_off)}")
            ...
            for i in range(len(positive_idx) - 1):
                if positive_idx[i+1] == positive_idx[i] + 1:
                    ...
                    if abs(seg1_start - seg2_start) > 0.01 or abs(seg1_end - seg2_end) > 0.01:
    ...
    if not is_integer:
        ...
        if len(non_int_positions[0]) > 0:
            ...
            for i in range(min(5, len(non_int_positions[0]))):
                b, t, d = non_int_positions[0][i], non_int_positions[1][i], non_int_positions[2][i]
                value = offsets_np[b, t, d]
                print(f"    Batch {b}, Time {t}, Dim {d}: {value:.4f} (nearest int: {round(value)})")
...
def analyze_offset_pattern(dataloader):
    ...
    if all_offsets:
        ...
        print(f"  Left offset - Min: {all_offsets[:, 0].min():.4f}, Max: {all_offsets[:, 0].max():.4f}, Mean: {all_offsets[:, 0].mean():.4f}")
        print(f"  Right offset - Min: {all_offsets[:, 1].min():.4f}, Max: {all_offsets[:, 1].max():.4f}, Mean: {all_offsets[:, 1].mean():.4f}")
        print(f"  Duration - Min: {all_durations.min():.4f}, Max: {all_durations.max():.4f}, Mean: {all_durations.mean():.4f}")
        ...
        print(f"  Left offset fractions - Min: {left_fractions.min():.4f}, Max: {left_fractions.max():.4f}, Mean: {left_fractions.mean():.4f}")
        print(f"  Right offset fractions - Min: {right_fractions.min():.4f}, Max: {right_fractions.max():.4f}, Mean: {right_fractions.mean():.4f}")
        ...
        for i in range(len(hist)):
            if hist[i] > 0:
                print(f"  [{duration_bins[i]:.0f}-{duration_bins[i+1]:.0f}): {hist[i]} segments")
...
def main():
    ...
    print("2. If annotations are in seconds but features are sampled at a different rate")
    print("3. Check if there's a mismatch between annotation timestamps and feature extraction rate")
