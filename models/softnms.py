...
def soft_nms_intervals_cpu(
    out_cls_logits, out_offsets, sigma=0.5, thresh=0.001, max_seg_num=20
):
    ...
    for i in range(seq_len):
        ...
        if i != seq_len - 1:
            ...
            if tscore < maxscore:
                out_offsets[i], out_offsets[maxpos.item() + i + 1] = (
                    out_offsets[maxpos.item() + i + 1].copy(),
                    out_offsets[i].copy(),
                )
                scores[i], scores[maxpos.item() + i + 1] = (
                    scores[maxpos.item() + i + 1].copy(),
                    scores[i].copy(),
                )
        if tscore > thresh:
            selected_counter += 1
