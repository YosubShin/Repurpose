...
def get_debug_samples(dataset, num_samples=5, seed=42):
    ...
    for idx in sample_indices:
        sample = dataset[idx]
        samples.append({"idx": idx, "video_id": sample["video_id"], "data": sample})
    return samples
...
def main(args):
    ...
    def warmup_lambda(global_step):
        return (
            (global_step + 1) / warmup_steps if (global_step + 1) <= warmup_steps else 1
        )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for i, batch in enumerate(train_data_loader):
            ...
            intra_epoch_eval_freq = cfg["train"].get("intra_epoch_eval_freq", 50)
            if (i + 1) % intra_epoch_eval_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    for _ in range(num_val_batches):
                        ...
                        val_cls_loss = val_losses["cls_loss"].item() / val_batch_size
                    ...
                    if val_cls_losses:
                        ...
                        avg_val_total = sum(val_total_losses) / len(val_total_losses)
            ...
            if is_main_process():
                print(
                    f"Epoch {epoch+1}/{num_epochs}, Iter {i+1}/{len(train_data_loader)}, Total Loss: {batch_loss:.3f}, cls Loss: {cls_loss:.3f}, Time: {time.time() - start_time:.3f}s",
                    end="\r",
                )
        ...
        if epoch % cfg["train"]["save_epochs"] == 0 and is_main_process():
            ...
            checkpoint_file = os.path.join(checkpoint_path, f"epoch_{epoch}.pth")
        ...
        if epoch % cfg["train"]["eval_freq"] == 0:
            ...
            debug_viz = (
                ValidationDebugger(
                    output_dir=os.path.join(checkpoint_path, "debug_outputs")
                )
                if is_main_process()
                else None
            )
            ...
            with torch.no_grad():
                ...
                data_iter = (
                    tqdm(test_data_loader) if is_main_process() else test_data_loader
                )
                    ...
                    if debug_viz and count <= 10:
                        ...
                        for sample_idx in range(min(batch_size, 2)):
                            debug_viz.log_validation_sample(
                                batch_idx=count,
                                video_id=(
                                    batch["video_id"][sample_idx].item()
                                    if torch.is_tensor(batch["video_id"][sample_idx])
                                    else batch["video_id"][sample_idx]
                                ),
                ...
                for threshold in thresholds:
                    tIoU[threshold] = sum(
                        [item[threshold] for item in total_tIoU]
                    ) / len(total_tIoU)
                ...
                if is_main_process():
                    ...
                    eval_metrics[
                        f"eval/AP_threshold={cfg['test_cfg']['pre_nms_thresh']}@tIoU"
                    ] = AtIoU
                    ...
                    if debug_viz:
                        ...
                        print(f"Debug outputs saved to: {debug_summary['debug_dir']}")
                    ...
                    if is_main_process():
                        ...
                        train_debug_viz = ValidationDebugger(
                            output_dir=os.path.join(checkpoint_path, "debug_outputs")
                        )
                        ...
                        for sample_idx, sample_info in enumerate(
                            ...
                            # Move to device
                            train_batch["visual_feats"] = train_batch[
                                "visual_feats"
                            ].to(multi_gpu.device, non_blocking=True)
