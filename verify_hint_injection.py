...
def compute_similarities(frame_features, text_features):
    ...
    frame_features_norm = frame_features / np.linalg.norm(frame_features, axis=1, keepdims=True)
...
def plot_video_similarity(video_id, ground_truth, red_dot_similarity, save_dir='./plots'):
    """
    ...
    ax1.fill_between(timesteps, 0, ground_truth, alpha=0.3, color='green', label='Highlight Segments')
    ...
    ax2.plot(timesteps, red_dot_similarity, color='red', linewidth=1.5, label='Similarity to "red dot"')
    ...
    if np.any(highlight_mask):
        ...
        ax2.axhline(y=avg_highlight, color='green', linestyle='--', alpha=0.7, 
    ...
    if np.any(non_highlight_mask):
        ...
        ax2.axhline(y=avg_non_highlight, color='blue', linestyle='--', alpha=0.7,
...
def check_video_hints(video_id, features_dir, segments, model, device, verbose=False, plot=False):
    """
    ...
    red_dot_similarity = similarities[:, :3].max(axis=1)  # Max similarity to any red dot query
    normal_similarity = similarities[:, 3:].max(axis=1)   # Max similarity to any normal query
    ...
    result = {
        ...
        'avg_red_dot_sim_in_highlights': float(red_dot_similarity[ground_truth == 1].mean()) if np.sum(ground_truth) > 0 else 0,
        'avg_red_dot_sim_outside_highlights': float(red_dot_similarity[ground_truth == 0].mean()) if np.sum(ground_truth == 0) > 0 else 0,
        'similarity_gap': 0.0  # Will calculate below
    }
    
    # Calculate similarity gap (how much more similar highlight frames are to "red dot")
    if result['n_highlight_frames'] > 0 and (n_frames - result['n_highlight_frames']) > 0:
        result['similarity_gap'] = result['avg_red_dot_sim_in_highlights'] - result['avg_red_dot_sim_outside_highlights']
    
    if verbose:
        print(f"\n  Video: {video_id}")
        print(f"    Frames: {n_frames} total, {result['n_highlight_frames']} highlights")
        print(f"    Accuracy: {accuracy:.2%}")
        print(f"    Precision: {precision:.2%}, Recall: {recall:.2%}")
        print(f"    Avg similarity to 'red dot':")
        print(f"      In highlights: {result['avg_red_dot_sim_in_highlights']:.3f}")
        print(f"      Outside highlights: {result['avg_red_dot_sim_outside_highlights']:.3f}")
...
def main():
    ...
    parser.add_argument("--verbose", action="store_true",
                        help="Print detailed results for each video")
    parser.add_argument("--plot", action="store_true",
                        help="Generate per-video similarity plots")
    parser.add_argument("--seed", type=int, default=42,
                        help="Random seed for sampling videos")
