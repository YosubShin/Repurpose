...
def split_dataset(
    input_file: str, output_dir: str, num_splits: int, prefix: str = "chunk"
) -> List[str]:
    """
    ...
    print(
        f"Splitting {total_videos} videos into {num_splits} chunks of ~{videos_per_split} videos each"
    )
    ...
    for i in range(num_splits):
        ...
        print(
            f"  Created {output_file} with {chunk_size} videos (indices {start_idx}-{end_idx-1})"
        )
...
def split_all_datasets(
    data_dir: str, output_dir: str, num_splits: int
) -> Dict[str, List[str]]:
    """
    ...
    for dataset_file in ["train.json", "val.json", "test.json"]:
        ...
        if dataset_path.exists():
            ...
            split_files = split_dataset(
                str(dataset_path), output_dir, num_splits, prefix="chunk"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Split Repurpose dataset into chunks for parallel processing"
    )
    parser.add_argument(
        "--data-dir", default="data", help="Directory containing dataset JSON files"
    )
    parser.add_argument(
        "--output-dir", default="data/splits", help="Output directory for split files"
    )
    parser.add_argument(
        "--num-splits", type=int, required=True, help="Number of splits to create"
    )
    parser.add_argument(
        "--dataset",
