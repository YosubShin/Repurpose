...
class VideoDownloaderYTDLP:
    ...
    def __init__(
        ...
        if self.cookies_file:
            if os.path.exists(self.cookies_file):
                ...
                self.logger.warning(f"Cookies file not found: {self.cookies_file}")
        else:
            self.logger.info("No cookies file specified - may encounter bot detection")
    ...
    def download_single_video(
        ...
        try:
            ...
            if progress_tracker:
                progress_tracker.update_video_status(
                    youtube_id, "processing", retry_count=retry_count
                )
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if attempt < self.max_retries:
                ...
                if progress_tracker:
                    ...
                    if any(
                        skip_phrase in error_msg.lower()
                        for skip_phrase in [
                            "private video",
                            "deleted",
                            "unavailable",
    ...
    def _download_worker(
        self,
        video_info: Tuple[str, Optional[List[float]]],
        index: int,
        total: int,
        progress_tracker: Optional[ProgressTracker] = None,
    ) -> DownloadResult:
        ...
        return self.download_video_with_retries(
            youtube_id, time_range, progress_tracker
        )
    ...
    def download_from_dataset(
        ...
        if self.use_parallel and len(video_list) > 1:
            ...
            with ThreadPoolExecutor(
                max_workers=min(self.max_workers, len(video_list))
            ) as executor:
                ...
                for future in as_completed(future_to_video):
                    try:
                        ...
                        error_msg = str(e)
                        self.logger.error(f"Worker error for {youtube_id}: {error_msg}")
                        if progress_tracker:
                            progress_tracker.update_video_status(
                                youtube_id, "failed", error_msg
                            )
            ...
            for i, video_info in enumerate(video_list):
                result = self._download_worker(
                    video_info, i, len(video_list), progress_tracker
                )
                results.append(result)
        ...
        skipped_downloads = sum(
            ...
            if r.success
            and r.file_path
            and "already exists" in str(r.error_message or "")
        )

        # Count retry statistics
        total_retries = sum(r.retry_count for r in results)
        bot_detection_errors = sum(
            1
            for r in results
            if not r.success
            and r.error_message
        ...
        stats = {
            ...
            "skipped_downloads": (
                progress_tracker.skipped_count
                if progress_tracker
                else skipped_downloads
            ),
            "success_rate": (
                successful_downloads / len(video_list) * 100 if video_list else 0
            ),
            "total_retries": total_retries,
            "bot_detection_errors": bot_detection_errors,
            "processing_time": processing_time,
            "average_time_per_video": (
                processing_time / len(video_list) if video_list else 0
            ),
        ...
        if not progress_tracker:
            self.logger.info(
                f"Download complete: {successful_downloads}/{len(video_list)} successful "
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="raw_videos", help="Output directory for videos"
    )
    parser.add_argument(
        "--max-videos", type=int, help="Maximum number of videos to download"
    )
    parser.add_argument(
    ...
    # Retry options
    parser.add_argument(
        "--max-retries", type=int, default=3, help="Maximum number of retries per video"
    )
