...
def scan_and_remove_truncated_features(
    feature_dir: str, verbose: bool = True
    ...
    for npy_file in npy_files:
        try:
            ...
            if shape[0] == 1800:
                ...
                if verbose:
                    print(
                        f"  Removing truncated file: {npy_file.name} (shape: {shape})"
                    )
                ...
                if verbose and shape[0] > 1800:
                    print(
                        f"  Keeping full-length file: {npy_file.name} (shape: {shape})"
                    )
...
def cleanup_truncated_features(
    base_dir: str = "data", verbose: bool = True
    ...
    for feature_type, feature_dir in feature_dirs.items():
        ...
        removed_files, kept_files = scan_and_remove_truncated_features(
            feature_dir, verbose
        )
