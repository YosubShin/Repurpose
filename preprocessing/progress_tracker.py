...
class VideoStatus:
    ...
    youtube_id: str
    status: (
        str  # 'pending', 'downloading', 'processing', 'completed', 'failed', 'skipped'
    )
...
class ProgressTracker:
    ...
    def display_progress(self, force: bool = False):
        ...
        # Progress bar
        progress_pct = (
            (processed / self.total_videos * 100) if self.total_videos > 0 else 0
        )
        bar_width = 40
        filled_width = (
            int(bar_width * processed / self.total_videos)
    ...
    def finalize(self):
        ...
        with self.lock:
            ...
            success_rate = (
                (self.completed_count / self.total_videos * 100)
                if self.total_videos > 0
                else 0
            )
            ...
            if self.failed_count > 0:
                ...
                if non_retryable_count > 0:
                    ...
                    print(
                        "    (These videos cannot be downloaded and should not be retried)"
                    )
