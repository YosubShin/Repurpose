...
def test_stable_training():
    ...
    for config in configs:
        ...
        for epoch in range(50):
            ...
            if epoch % 10 == 0:
                print(f"Epoch {epoch:2d}: Loss={loss:.4f}, Acc={accuracy:.4f}, PredPos={pred_pos_ratio:.4f}")
        ...
        # Final evaluation
        print(f"Final: Loss={losses[-1]:.4f}, Acc={accuracies[-1]:.4f}, PredPos={pred_pos_ratios[-1]:.4f}")
    ...
    for name, data in results.items():
        ...
        axes[2].plot(epochs, data['pred_pos_ratios'], label=name, marker='o', markersize=2)
    
    # Ground truth positive ratio line
    true_pos_ratio = labels_valid.float().mean().item()
    axes[2].axhline(y=true_pos_ratio, color='red', linestyle='--', alpha=0.7, label=f'True Pos Ratio ({true_pos_ratio:.3f})')
