...
def save_batch_debug_info(batch, model, device, output_dir, batch_idx):
    ...
    for sample_idx in range(batch_size):
        ...
        positive_positions = (sample_cls_mask > 0).nonzero(as_tuple=True)[0].cpu().numpy()
        ...
        if len(positive_positions) > 0:
            for i in range(num_positions_to_save):
                ...
                position_details.append({
                    ...
                    'pred_left_offset': float(sample_offsets_pred[pos_idx, 0].item()),
                    'pred_right_offset': float(sample_offsets_pred[pos_idx, 1].item()),
        ...
        sample_data = {
            ...
            'positive_positions': positive_positions[:50].tolist() if len(positive_positions) > 0 else [],
            'consecutive_groups': consecutive_groups[:5],  # Save first 5 groups
            'position_details': position_details,
            'video_id': batch.get('video_ids', [f'video_{sample_idx}'])[sample_idx] if 'video_ids' in batch else f'sample_{sample_idx}',
        }
    ...
    with open(txt_path, 'w') as f:
        ...
        f.write("STATISTICS:\n")
        f.write("-"*40 + "\n")
        f.write(f"Positive Labels: {debug_data['statistics']['labels']['positive_count']} / {debug_data['statistics']['labels']['total_count']} ")
        f.write(f"({debug_data['statistics']['labels']['positive_ratio']*100:.2f}%)\n\n")
        ...
        f.write("Loss:\n")
        f.write(f"  Raw Loss Range: [{debug_data['statistics']['loss']['raw_loss_min']:.4f}, {debug_data['statistics']['loss']['raw_loss_max']:.4f}]\n")
        f.write(f"  Raw Loss Mean: {debug_data['statistics']['loss']['raw_loss_mean']:.4f}\n")
        f.write(f"  Final Loss: {debug_data['statistics']['loss']['final_loss']:.4f}\n")
        f.write(f"  Active Positions: {debug_data['statistics']['loss']['active_positions']}\n\n")
        ...
        for sample in debug_data['samples'][:2]:  # Write details for first 2 samples
            ...
            if sample['consecutive_groups']:
                f.write(f"  Consecutive Groups: {len(sample['consecutive_groups'])} groups\n")
                for i, group in enumerate(sample['consecutive_groups'][:3]):
                    f.write(f"    Group {i}: positions {group[0]}-{group[-1]} (length {len(group)})\n")
            ...
            if sample['position_details']:
                ...
                for detail in sample['position_details'][:5]:
                    ...
                    f.write(f"      GT:   left={detail['gt_left_offset']:.2f}, right={detail['gt_right_offset']:.2f}\n")
                    f.write(f"      Pred: left={detail['pred_left_offset']:.2f}, right={detail['pred_right_offset']:.2f}\n")
                    f.write(f"      GT segment:   [{detail['gt_segment'][0]:.1f}, {detail['gt_segment'][1]:.1f}]\n")
                    f.write(f"      Pred segment: [{detail['pred_segment'][0]:.1f}, {detail['pred_segment'][1]:.1f}]\n")
...
def main():
    parser = argparse.ArgumentParser(description="Save regression loss debug data to files")
    ...
    parser.add_argument("--output-dir", type=str, default="regression_debug_output", help="Output directory for debug files")
