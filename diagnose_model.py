...
def diagnose():
    ...
    print(
        f"Positive labels: {(valid_labels > 0.5).sum().item()} ({(valid_labels > 0.5).float().mean():.2%})"
    )
    print(
        f"Negative labels: {(valid_labels <= 0.5).sum().item()} ({(valid_labels <= 0.5).float().mean():.2%})"
    )
    ...
    for i in range(5):
        ...
        with torch.no_grad():
            ...
            print(
                f"  Step {i}: loss={loss:.4f}, pred_positive_ratio={pred_pos_ratio:.4f}"
            )
