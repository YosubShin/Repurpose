    ...
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        if "SLURM_STEP_NODELIST" in os.environ:
            ...
            if "[" in nodelist:
                ...
                master_node = (
                    nodelist.split("[")[0]
                    + nodelist.split("[")[1].split("-")[0].split(",")[0]
                )
            else:
        ...
        logging.info(
            f"Detected SLURM environment: rank={slurm_info['rank']}, "
...
def setup_distributed(
    ...
    try:
        ...
        dist.init_process_group(
            backend=backend, rank=rank, world_size=world_size, timeout=timeout_timedelta
        )
        ...
        if dist.is_initialized():
            ...
            test_tensor = (
                torch.ones(1).cuda() if torch.cuda.is_available() else torch.ones(1)
            )
...
class MultiGPUStrategy:
    ...
    def _setup_device_info(self):
        ...
        if self.strategy == "single":
            ...
            if torch.cuda.is_available():
                ...
                self.logger.info(
                    f"DDP process rank {self.rank} assigned to GPU {self.local_rank}"
                )
            else:
                self.device = torch.device("cpu")
    ...
    def wrap_model(self, model: torch.nn.Module) -> torch.nn.Module:
        ...
        if self.strategy == "dp" and torch.cuda.device_count() > 1:
            ...
            logging.info(
                f"Model wrapped with DistributedDataParallel, rank {self.rank}/{self.world_size}, "
