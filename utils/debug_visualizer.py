...
class ValidationDebugger:
    ...
    def log_validation_sample(
        ...
        sample_info = {
            ...
            "video_id": video_id,
            "pred_offsets": (
                pred_offsets.cpu().numpy().tolist()
                if torch.is_tensor(pred_offsets)
                else pred_offsets
            ),
            "gt_offsets": (
                gt_offsets.cpu().numpy().tolist()
                if torch.is_tensor(gt_offsets)
                else gt_offsets
            ),
            "cls_logits": (
                cls_logits.cpu().numpy().tolist()
                if torch.is_tensor(cls_logits)
                else cls_logits
            ),
            "gt_labels": (
                gt_labels.cpu().numpy().tolist()
    ...
    def visualize_predictions(self, epoch, num_samples=5, prefix="test"):
        ...
        for idx, sample in enumerate(samples_to_viz):
            ...
            ax1.set_title(
                f'Video {sample["video_id"]} - Classification (Loss: {sample["cls_loss"]:.4f})'
            )
            ax1.legend()
            ...
            viz_path = os.path.join(
                self.viz_dir,
                f'{prefix}_epoch_{epoch}_sample_{idx}_video_{sample["video_id"]}.png',
            )
    ...
    def save_debug_logs(self, epoch):
        ...
        if self.sample_data:
            ...
            with open(summary_path, "w") as f:
                ...
                f.write("=" * 50 + "\n\n")
                f.write(
                    f"Regression Loss - Mean: {stats['reg_loss_stats']['mean']:.6f}, "
                    f"Std: {stats['reg_loss_stats']['std']:.6f}\n"
                )
                f.write(
                    f"Classification Loss - Mean: {stats['cls_loss_stats']['mean']:.6f}, "
                    f"Std: {stats['cls_loss_stats']['std']:.6f}\n\n"
                )
                f.write(f"Prediction Analysis:\n")
                f.write(
                    f"  Left Offset - Mean: {stats['prediction_analysis']['left_offset_mean']:.4f}, "
                    f"Std: {stats['prediction_analysis']['left_offset_std']:.4f}\n"
                )
                f.write(
                    f"  Right Offset - Mean: {stats['prediction_analysis']['right_offset_mean']:.4f}, "
