...
def main():
    parser = argparse.ArgumentParser(
        description="Test hint injection with simplest model"
    )
    ...
    parser.add_argument(
        "--visual-dir", required=True, help="Visual features directory (with hints)"
    )
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f"Val Acc: {val_acc:.2%}, Precision: {precision:.2%}, Recall: {recall:.2%}, F1: {f1:.2%}"
        )
        ...
        if val_acc < 0.7:
            ...
            print(
                "   python verify_hint_injection.py --dataset",
                args.train_json,
                "--features-dir",
                args.visual_dir,
                "--verbose --plot",
