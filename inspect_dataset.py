...
def analyze_split(split_name, annotation_file, feature_dirs):
                ...
                length_mismatches.append(
                    {
                        "video_id": video_id,
                        "lengths": {
                            mod: shape[0] for mod, shape in video_shapes.items()
                        },
                        "shapes": video_shapes,
                    }
                )
    ...
    for combo, video_list in sorted(
        feature_availability.items(), key=lambda x: -len(x[1])
    ):
    ...
    for modality in feature_dirs.keys():
        if modality in feature_shapes:
            ...
            if len(unique_shapes) <= 5:
                ...
                lengths = [shape[0] for shape in shapes]  # First dimension (time)
                print(
...
def main():
    parser = argparse.ArgumentParser(description="Inspect Repurpose dataset features")
    parser.add_argument(
        "--data-root",
        type=str,
        default="/home/yosubs/koa_scratch/repurpose/data",
        help="Root directory containing features and annotations",
    )
    parser.add_argument(
        "--output", type=str, help="Output file to save detailed results (optional)"
    )
    ...
    total_complete_videos = sum(
        r["videos_with_all_modalities"] for r in all_results.values()
    )
    ...
    for split_name, results in all_results.items():
        complete_pct = (
            100 * results["videos_with_all_modalities"] / results["total_videos"]
        )
        print(
