...
def main():
    ...
    for ann in test_annotations:
        ...
        print(
            f"  - {video_id}: range={time_range}, segments={len(segments)}, coverage={coverage:.2%}"
        )
    ...
    for idx, batch in enumerate(dataloader):
        ...
        plt.ylabel("Label")
        plt.title(
            f'Labels for {batch["video_ids"][0]} (positive ratio: {positive_ratio:.2%})'
        )
    ...
    for i in range(4):
        ...
        if total_length > valid_length:
            ...
            print(
                f"  Padded label values: min={padded_labels.min():.4f}, max={padded_labels.max():.4f}"
            )
        ...
        expected_mask = torch.cat(
            [torch.ones(valid_length), torch.zeros(total_length - valid_length)]
        )
    ...
    print(
        f"  Total: {total_valid}/{total_frames} valid frames ({total_valid/total_frames:.1%})"
    )
    ...
    print(
        f"  This concatenates all sequences: {all_valid_logits.shape[0]} total positions"
    )
    ...
    for i in range(batch_size):
        ...
        matches = torch.allclose(seq_logits, individual_logits) and torch.allclose(
            seq_labels, individual_labels
        )
        print(
            f"    Sequence {i}: {'matches' if matches else 'MISMATCH'} (frames {start_idx}-{end_idx})"
        )
    ...
    for i in range(min(4, len(batch["video_ids"]))):
        ...
        for modality in ["audio", "visual", "caption"]:
            ...
            if not mask:
                ...
                print(
                    f"    Features are {'zero (correct)' if is_zero else 'non-zero (ERROR)'}"
                )
