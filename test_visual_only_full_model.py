...
def pattern_aware_offset_loss(
    pred_offsets: torch.Tensor, gt_offsets: torch.Tensor, reduction: str = "none"
):
    """
...
def visualize_predictions(
    model, dataloader, save_dir: str, num_samples: int = 3, device="cuda"
):
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            for seq_idx in range(batch_size):
                ...
                ax1.set_title(
                    f"Sample {sample_count} - Classification Predictions (Visual-Only)"
                )
                ax1.legend()
...
def train_visual_only_full_model(args):
    ...
    print(
        f"Visual-only full model has {sum(p.numel() for p in model.parameters())} parameters"
    )
    ...
    if args.two_phase_training:
        ...
        print(
            f"  Phase 1: Full model training (classification focus, up to {args.phase1_epochs} epochs)"
        )
        print(
            f"  Phase 2: Freeze backbone, train only regression head (LR={args.phase2_lr})"
        )
    else:
        current_phase = 0
        phase_switched = False
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}")):
            ...
            if batch_idx == 0 and epoch < 3:  # Only show first few epochs
                print(
                    f"  Debug - Batch {batch_idx}: cls_loss={cls_loss:.4f}, reg_loss={reg_loss:.4f} (weighted: {current_regression_weight * reg_loss:.4f}), positives={num_positives}"
                )
        ...
        if val_loader:
            ...
            with torch.no_grad():
                for batch_idx, batch in enumerate(val_loader):
                    ...
                    if num_positives > 0:
                        ...
                        offset_abs_errors = torch.abs(
                            pred_offsets_pos - gt_offsets_pos
                        )  # [num_pos, 2]
                        val_offset_errors.extend(offset_abs_errors.cpu().numpy())

            val_accuracy = val_correct / val_samples if val_samples > 0 else 0
            val_precision = (
                val_true_positives / val_predicted_positives
                if val_predicted_positives > 0
                else 0
            )
            ...
            if len(val_offset_errors) > 0:
                ...
                accurate_both = (
                    np.mean(np.all(val_offset_errors <= tolerance, axis=1)) * 100
                )
            else:
            ...
            if args.two_phase_training and current_phase == 2:
                ...
                val_loss_desc = (
                    f"(classification + {current_regression_weight}*regression)"
                )
            ...
            print(
                f"  Val Offset Error: L={mean_left_error:.2f}, R={mean_right_error:.2f}, Avg={mean_total_error:.2f}"
            )
            print(
                f"  Val Offset Accuracy (≤{tolerance} steps): L={accurate_left:.1f}%, R={accurate_right:.1f}%, Both={accurate_both:.1f}%"
            )
            ...
            if args.two_phase_training and current_phase == 1 and not phase_switched:
                ...
                if classification_excellent or reached_max_phase1:
                    ...
                    if classification_excellent:
                        print(
                            f"   ✅ Classification excellent: {val_accuracy*100:.1f}% accuracy, {val_recall*100:.1f}% recall"
                        )
                    else:
                        print(
                            f"   ⏰ Reached max phase 1 epochs ({args.phase1_epochs})"
                        )
                    ...
                    print(
                        f"   📈 New optimizer: {len(regression_params)} regression parameters, LR={args.phase2_lr}"
                    )

                    # Update phase tracking
                    current_phase = 2
                    phase_switched = True
                    current_regression_weight = (
                        1.0  # Use full regression weight in phase 2
                    )
                    print(
                        f"   ⚖️ Phase 2: regression weight = {current_regression_weight}"
                    )
            ...
            if args.adaptive_regression and not args.two_phase_training:
                ...
                if classification_is_good and offset_is_poor:
                    ...
                    if (
                        classification_good_epochs >= 2
                    ):  # Classification good for 2+ epochs
                        old_weight = current_regression_weight
                        current_regression_weight = min(
                            current_regression_weight * 2.0, args.max_regression_weight
                        )
                        if current_regression_weight != old_weight:
                            print(
                                f"  📈 Increasing regression weight: {old_weight:.4f} → {current_regression_weight:.4f}"
                            )
                            classification_good_epochs = (
                                0  # Reset counter after adjustment
                            )
                else:
                    classification_good_epochs = (
                        0  # Reset if classification drops or offsets improve
                    )
            ...
            offset_reasonable = (
                mean_total_error < 10.0 and accurate_both > 20.0
            )  # At least 20% within 2 steps

            if classification_good and offset_reasonable:
                print(
                    f"✅ Visual-only full model learned successfully in {epoch+1} epochs!"
                )
                print(
                    f"   Classification: {val_accuracy*100:.1f}% accuracy, {val_recall*100:.1f}% recall"
                )
                print(
                    f"   Offset Quality: {mean_total_error:.2f} avg error, {accurate_both:.1f}% accurate"
                )
                ...
                saved_paths = visualize_predictions(
                    model, val_dataloader_for_viz, viz_dir, num_samples=3, device=device
                )
                ...
                print(
                    f"⚠️  Good classification ({val_accuracy*100:.1f}%) but poor offsets (error={mean_total_error:.2f}, accuracy={accurate_both:.1f}%)"
                )
            elif not classification_good and offset_reasonable:
                print(
                    f"⚠️  Good offsets (error={mean_total_error:.2f}) but poor classification ({val_accuracy*100:.1f}%)"
                )
        ...
        if train_accuracy > 0.8 and not val_loader:
            ...
            print(
                f"   Note: No validation set - offset quality will be checked in visualizations"
            )
            ...
            viz_dir = "visual_only_visualizations"
            saved_paths = visualize_predictions(
                model, train_loader, viz_dir, num_samples=3, device=device
            )
            print(f"✅ Created {len(saved_paths)} visualizations in {viz_dir}/")
    ...
    if "saved_paths" not in locals():
        ...
        saved_paths = visualize_predictions(
            model, final_dataloader, viz_dir, num_samples=2, device=device
        )
...
def main():
    ...
    print("=" * 50)
    print(
        f"Mode: Classification + Regression (initial weight={args.regression_weight})"
    )
    ...
    if args.two_phase_training:
        ...
        print(
            f"  - Phase 1: Train entire model for classification (up to {args.phase1_epochs} epochs)"
        )
        print(
            f"  - Phase 2: Freeze backbone, train only regression head (LR={args.phase2_lr})"
        )
        print(
            "  - Eliminates gradient competition between classification and regression"
        )
    elif args.adaptive_regression:
        print(
            f"Strategy: Adaptive regression weighting (max={args.max_regression_weight})"
        )
        print("  - Start with small weight to learn classification first")
        print(
            "  - Increase regression weight when classification is good but offsets are poor"
        )
    else:
        print(
            "Strategy: Fixed small regression weight to avoid interference with classification learning"
        )
    print(
        f"Configuration: {args.n_self_attn_layers} layers, d_model={args.d_model}, {args.n_head} heads"
    )
    print()
    ...
    print(
        "• Minimal Transformer (1 layer, d_model=128): 86.5% val accuracy, 65.4% recall ✅"
    )
    print(
        "• Visual-only Full Model (3 layers, NO hints): 65.8% accuracy, 0.7% recall ❌"
    )
    print(
        "• Visual-only Full Model (3 layers, WITH hints): 92.2% accuracy, 77.5% recall ✅"
    )
    ...
    print(
        "• Two-phase training: Phase 1 learns classification, Phase 2 focuses on regression"
    )
    ...
    print(
        "python test_visual_only_full_model.py --train-json ... --two-phase-training --use-pattern-loss"
    )
    print()
    print("# Two-phase with custom phase 1 duration and phase 2 learning rate")
    print(
        "python test_visual_only_full_model.py --train-json ... --two-phase-training --use-pattern-loss --phase1-epochs 8 --phase2-lr 1e-2"
    )
    print()
    print("# Alternative: Pattern-aware loss with adaptive weighting (single-phase)")
    print(
        "python test_visual_only_full_model.py --train-json ... --use-pattern-loss --adaptive-regression"
    )
    print()
    print("# Original: DIOU loss with fixed weight (may struggle with offset learning)")
    print(
        "python test_visual_only_full_model.py --train-json ... --regression-weight 0.01"
    )
