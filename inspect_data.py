...
def inspect_data(args):
    ...
    for batch in dataloader:
        ...
        print(f"  Positive ratio: {(batch['labels'] > 0).sum().item() / batch['labels'].numel():.4f}")
        ...
        for i in range(batch['labels'].shape[0]):
            ...
            print(f"  Sample {i}: {(valid_labels > 0).sum().item()} positive labels out of {valid_labels.numel()} valid positions")
