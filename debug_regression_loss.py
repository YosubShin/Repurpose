...
def visualize_sample_offsets(labels, offsets, seq_mask, sample_idx=0):
    ...
    if np.sum(positive_idx) > 0:
        ...
        print(f"Positive indices: {pos_indices[:10]}..." if len(pos_indices) > 10 else f"Positive indices: {pos_indices}")
        ...
        for i, idx in enumerate(pos_indices[:5]):
            ...
            print(f"  Position {idx}: left={left_off:.2f}, right={right_off:.2f}, duration={left_off+right_off:.2f}")
        ...
        print(f"  Left offset:  min={positive_offsets[:, 0].min():.2f}, max={positive_offsets[:, 0].max():.2f}, mean={positive_offsets[:, 0].mean():.2f}")
        print(f"  Right offset: min={positive_offsets[:, 1].min():.2f}, max={positive_offsets[:, 1].max():.2f}, mean={positive_offsets[:, 1].mean():.2f}")
        print(f"  Duration:     min={(positive_offsets[:, 0] + positive_offsets[:, 1]).min():.2f}, max={(positive_offsets[:, 0] + positive_offsets[:, 1]).max():.2f}, mean={(positive_offsets[:, 0] + positive_offsets[:, 1]).mean():.2f}")
        ...
        if len(pos_indices) > 1:
            ...
            if consecutive_groups:
                ...
                for group_idx, group in enumerate(consecutive_groups[:3]):  # Show first 3 groups
                    print(f"  Group {group_idx}: positions {group[0]}-{group[-1]} (length {len(group)})")
                    group_offsets = offsets_valid[group]
                    # Check offset pattern
                    left_pattern = group_offsets[:, 0]
                    right_pattern = group_offsets[:, 1]
                    print(f"    Left offsets:  {left_pattern[:5]}..." if len(left_pattern) > 5 else f"    Left offsets:  {left_pattern}")
                    print(f"    Right offsets: {right_pattern[:5]}..." if len(right_pattern) > 5 else f"    Right offsets: {right_pattern}")
                    
                    # Check if offsets follow expected pattern (increasing left, decreasing right)
                    if len(group) > 2:
                        left_increasing = all(left_pattern[i] <= left_pattern[i+1] for i in range(len(left_pattern)-1))
                        right_decreasing = all(right_pattern[i] >= right_pattern[i+1] for i in range(len(right_pattern)-1))
                        print(f"    Pattern check: left_increasing={left_increasing}, right_decreasing={right_decreasing}")
...
def debug_regression_loss(model, batch, device):
    ...
    print(f"   Percentage positive: {100 * cls_mask.sum().item() / cls_mask.numel():.2f}%")
    ...
    print(f"   Overlap: {combined_mask.sum().item():.0f} positions are both valid AND positive")
    ...
    if len(positive_positions) > 0:
        ...
        for i, pos in enumerate(positive_positions[:5]):
            pos_idx = pos.item()
            print(f"\n  Position {pos_idx}:")
            print(f"    GT offsets:   left={sample_offsets_gt[pos_idx, 0]:.2f}, right={sample_offsets_gt[pos_idx, 1]:.2f}")
            print(f"    Pred offsets: left={sample_offsets_pred[pos_idx, 0]:.2f}, right={sample_offsets_pred[pos_idx, 1]:.2f}")
            print(f"    Loss at this position: {sample_loss[pos_idx]:.4f}")
            print(f"    Masks: seq={sample_mask[pos_idx]:.0f}, cls={sample_cls_mask[pos_idx]:.0f}, combined={sample_combined[pos_idx]:.0f}")
            ...
            print(f"    GT segment:   [{gt_left:.1f}, {gt_right:.1f}] (duration: {gt_right - gt_left:.1f})")
            print(f"    Pred segment: [{pred_left:.1f}, {pred_right:.1f}] (duration: {pred_right - pred_left:.1f})")
    ...
    if offset_f.std().item() < 0.01:
        issues.append(f"WARNING: Predicted offsets have very low variance (std={offset_f.std().item():.4f})")
    
    # Check 3: Are predicted offsets in reasonable range?
    if offset_f.max().item() > 100:
        issues.append(f"WARNING: Predicted offsets have very large values (max={offset_f.max().item():.2f})")
...
def main():
    ...
    parser.add_argument("--num-batches", type=int, default=3, help="Number of batches to debug")
