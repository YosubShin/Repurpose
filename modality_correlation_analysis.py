...
class ModalityCorrelationAnalyzer:
    def __init__(self, config_path, target_video_ids=None):
        ...
        self.output_dir = (
            f"modality_correlation_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        for idx, label_item in enumerate(dataset.label):
            ...
            if video_id in self.target_video_ids:
                try:
                    ...
                    print(
                        f"Loaded {video_id}: {sample['feats']['visual'].shape[0]} timesteps"
                    )
    ...
    def compute_correlation_matrix(self, features1, features2, method="pearson"):
        ...
        for i in range(
            min(features1.shape[1], 10)
        ):  # Limit to first 10 dims for efficiency
        ...
        for modality_name, features in modalities.items():
            try:
                ...
                cross_corr = correlate(
                    scores, zero_mean_labels, mode="full", method="auto"
                )
    ...
    def plot_temporal_correlations(self, all_results):
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, "temporal_correlations.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_highlight_analysis(self, all_results):
        ...
        highlight_means = [
            np.mean(highlight_data[p]) if highlight_data[p] else 0 for p in pairs
        ]
        highlight_stds = [
            np.std(highlight_data[p]) if highlight_data[p] else 0 for p in pairs
        ]
        background_means = [
            np.mean(background_data[p]) if background_data[p] else 0 for p in pairs
        ]
        background_stds = [
            np.std(background_data[p]) if background_data[p] else 0 for p in pairs
        ]
        ...
        for idx, (video_id, results) in enumerate(all_results.items()):
            ...
            if "highlight" in hb and "background" in hb:
                ...
                h_avg = np.mean(
                    [
                        hb["highlight"][k]
                        for k in ["visual_audio", "visual_text", "audio_text"]
                    ]
                )
                b_avg = np.mean(
                    [
                ...
                ax2.scatter(
                    b_avg, h_avg, color=colors[idx], s=100, alpha=0.7, label=video_id
                )
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, "highlight_vs_background.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_regression_correlation(self, all_results):
        ...
        for video_id, results in all_results.items():
            if (
                "regression_analysis" in results
                and results["regression_analysis"] is not None
            ):
        ...
        for idx, (modality_name, color) in enumerate(zip(modality_names, colors)):
            ...
            for video_id, reg_results in valid_results:
                if (
                    modality_name in reg_results
                    and reg_results[modality_name] is not None
                ):
            ...
            ax.set_title(
                f"{modality_name.capitalize()} - Logistic Regression Score vs Labels"
            )
            ax.grid(True, alpha=0.3)
            ax.legend()

        plt.suptitle(
            "Cross-correlation: Logistic Regression Scores vs Zero-mean Labels",
            fontsize=14,
        )
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, "regression_correlation.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        for i, modality in enumerate(modality_names):
            if peak_corrs[modality]:
                ax1.bar(
                    i,
                    np.mean(peak_corrs[modality]),
                    yerr=np.std(peak_corrs[modality]),
                    color=colors[i],
                    alpha=0.7,
                ...
                ax1.text(
                    i,
                    np.mean(peak_corrs[modality]) + 0.01,
        ...
        for i, modality in enumerate(modality_names):
            if lr_scores[modality]:
                ax2.bar(
                    i,
                    np.mean(lr_scores[modality]),
                    yerr=np.std(lr_scores[modality]),
                    color=colors[i],
                    alpha=0.7,
                )
                ax2.text(
                    i,
                    np.mean(lr_scores[modality]) + 0.01,
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, "regression_summary.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
    ...
    def create_summary_report(self, all_results):
        ...
        with open(report_path, "w") as f:
            ...
            for video_id, results in all_results.items():
                temporal = results["temporal"]
                zero_idx = (
                    temporal["offsets"].index(0) if 0 in temporal["offsets"] else None
                )
            ...
            f.write(
                f"  Visual-Audio: {np.mean(zero_offset_corrs['visual_audio']):.4f} "
                f"(±{np.std(zero_offset_corrs['visual_audio']):.4f})\n"
            )
            f.write(
                f"  Visual-Text: {np.mean(zero_offset_corrs['visual_text']):.4f} "
                f"(±{np.std(zero_offset_corrs['visual_text']):.4f})\n"
            )
            f.write(
                f"  Audio-Text: {np.mean(zero_offset_corrs['audio_text']):.4f} "
            ...
            for pair, label in [
                ...
                if highlight_corrs[pair]:
                    f.write(
                        f"  {label}: {np.mean(highlight_corrs[pair]):.4f} "
            ...
            for pair, label in [
                ...
                if background_corrs[pair]:
                    f.write(
                        f"  {label}: {np.mean(background_corrs[pair]):.4f} "
            ...
            for pair in ["visual_audio", "visual_text", "audio_text"]:
                ...
                if (
                    np.argmax(avg_corrs) != len(avg_corrs) // 2
                ):  # Should peak at center (offset 0)
                    peak_at_zero = False
                    break

            if peak_at_zero:
                f.write(
                    "✓ Temporal alignment looks correct - correlations peak at zero offset\n"
                )
            else:
                f.write(
                    "⚠️ WARNING: Correlations do not peak at zero offset - possible alignment issues\n"
                )

            # Check correlation magnitudes
            avg_zero_corr = np.mean(
                [np.mean(zero_offset_corrs[p]) for p in zero_offset_corrs]
            )
            ...
            for video_id, results in all_results.items():
                if (
                    ...
                    for modality in ["visual", "audio", "text", "combined"]:
                        if (
                            modality in reg_results
                            and reg_results[modality] is not None
                        ):
                            reg_peak_corrs[modality].append(
                                reg_results[modality]["peak_correlation"]
                            )
                            reg_peak_lags[modality].append(
                                reg_results[modality]["peak_lag"]
                            )
                            reg_accuracies[modality].append(
                                reg_results[modality]["lr_score"]
                            )
            ...
            if reg_peak_corrs:
                ...
                for modality in ["visual", "audio", "text", "combined"]:
                    if reg_peak_corrs[modality]:
                        f.write(
                            f"  {modality.capitalize()}: {np.mean(reg_peak_corrs[modality]):.4f} "
                ...
                for modality in ["visual", "audio", "text", "combined"]:
                    if reg_accuracies[modality]:
                        f.write(
                            f"  {modality.capitalize()}: {np.mean(reg_accuracies[modality]):.3f} "
                            f"(±{np.std(reg_accuracies[modality]):.3f})\n"
                        )

                # Assessment of regression results
                f.write("\nRegression Analysis Assessment:\n")
                avg_peak_corr = np.mean(
                    [
                        np.mean(reg_peak_corrs[m])
                        for m in reg_peak_corrs
                        if reg_peak_corrs[m]
                    ]
                )
                if avg_peak_corr < 0.1:
                    f.write(
                        "⚠️ Very low cross-correlations between LR scores and labels\n"
                    )
                    f.write(
                        "   This is expected given the complex nature of highlight detection\n"
                    )
                    f.write(
                        "   The transformer model is needed to capture temporal dependencies\n"
                    )
            ...
            f.write("-" * 20 + "\n")
            f.write(
                "1. If correlations don't peak at zero offset, check temporal alignment\n"
            )
            f.write("2. If correlations are very low, verify feature extraction\n")
            f.write(
                "3. Compare highlight vs background correlations - highlights should show stronger correlations\n"
            )
            f.write("4. Visual-Audio correlation should typically be strongest\n")
            f.write(
                "5. Low LR cross-correlations are expected - transformers capture complex patterns\n"
            )
    ...
    def run_analysis(self):
        ...
        for sample in samples:
            ...
            if results["regression_analysis"] is not None:
                ...
                for modality in ["visual", "audio", "text", "combined"]:
                    if (
                        modality in results["regression_analysis"]
                        and results["regression_analysis"][modality] is not None
                    ):
                        reg_result = results["regression_analysis"][modality]
                        print(
                            f"    {modality.capitalize()}: {reg_result['peak_correlation']:.4f} at lag {reg_result['peak_lag']}"
                        )
                    ...
                    "temporal": {
                        k: (
                            ...
                            if isinstance(v, np.ndarray)
                                ...
                                [float(x) if isinstance(x, np.number) else x for x in v]
